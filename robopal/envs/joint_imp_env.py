...
class ImpedGymEnv(SingleArmEnv):
    """ This env utilizes impedance controller. """
    ...
    def __init__(self,                  
        ...
        # 末端执行器初始位置
        self.init_pos, self.init_rot = self.kdl_solver.fk(self.mj_data.qpos[:7])
...
if __name__ == "__main__":
    ...
    for i in range(30000):
        print(i)
        if 500 < i < 1000:
            env.mj_data.xfrc_applied[7][0] = 30  # 表示连杆所受外力的力矩，第一维是每个body的ID，第二维有六个量（x，y，z方向的受力，后边是力矩）
        else:
            env.mj_data.xfrc_applied[7][0] = 0  # 还原数值，不施加外力
